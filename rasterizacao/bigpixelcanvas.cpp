...
void BigPixelCanvas::DrawLine(const wxPoint& p0, const wxPoint& p1, wxDC& dc)
{
    ...
    if(p0.x>p1.x){ //segundo ou terceiro quadrante
        dx=-dx;
        if(p0.y>p1.y){ // terceiro quadrante todo
            dy=-dy;
            x=p1.x;
            y=p1.y;
            limY=p0.y;
    ...
    else if(p0.y>p1.y){ // quarto quadrante
        dy=-dy;
        if(dy>dx){ // setimo octante
            x=p1.x;
            y=p1.y;
            limY=p0.y;
            incrementoX=-1;
